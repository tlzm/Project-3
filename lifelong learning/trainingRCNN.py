...
def reshapeFeaturestrain(id_df, seq_length, Feature):
    """
...
def reshapeFeaturesval(id_df, seq_length, Feature):
    """
    Only sequences that meet the window-length are considered, no padding is used. This means for testing
    we need to drop those which are below the window-length.
    """
...
fea_gen = (list(reshapeFeaturestrain(train_data[train_data['id']==id], sequence_length, feature_col)) for id in range(1, n_time + 1))

# generate sequences and convert to numpy array
fea_array = np.concatenate(list(fea_gen)).astype(np.float32)


feaval_gen = (list(reshapeFeaturesval(train_data[train_data['id']==id], sequence_length, feature_col)) for id in range(1, n_time + 1))
feaval_array = np.concatenate(list(feaval_gen)).astype(np.float32)
...
print("The val data set has now shape: {} entries, {} time windows and {} features.".format(feaval_array.shape[0],feaval_array.shape[1],feaval_array.shape[2]))
# function to generate label
def reshapeLabeltrain(id_df, seq_length=sequence_length, label=['RUL']):
    """ Only sequences that meet the window-length are considered, no padding is used. This means for testing
    we need to drop those which are below the window-length."""
...
def reshapeLabelval(id_df, seq_length=sequence_length, label=['RUL']):
    """ Only sequences that meet the window-length are considered, no padding is used. This means for testing
    we need to drop those which are below the window-length."""
    data_matrix = id_df[label].values
    num_elements = data_matrix.shape[0]
    return data_matrix[num_elements-1: num_elements, :]

# generate labels
label_gen = [reshapeLabeltrain(train_data[train_data['id']==id]) for id in range(1, n_time + 1)]

label_array = np.concatenate(label_gen).astype(np.float32)

labelval_gen = [reshapeLabelval(train_data[train_data['id']==id]) for id in range(1, n_time + 1)]

labelval_array = np.concatenate(labelval_gen).astype(np.float32)


print(label_array.shape)
print(labelval_array.shape)
# MODEL
